...
- **Ограничения и компромиссы:**
  - **Smoke test (R27.10) не запускался операционно.** Задача предполагала прогон `features_build.py` на реальных NHL данных с подсчётом числа фичей (~280–350). Unit-тесты (615 шт.) охватывают логику генераторов, но E2E с реальными parquet-файлами не проверялся в рамках этого цикла.
  ...
  - **Streak-генератор — O(n²) по `long["id"] == mid`** — линейный поиск по id матча для каждой строки. На типичных сезонных данных (< 10 000 матчей) незначительно, но масштабируется плохо.
...
- **Возможные улучшения / техдолг:**
  - Запустить R27.10 (операционный smoke test) при следующем запуске пайплайна NHL; сравнить количество `f_`-колонок с базовыми 195 и убедиться в отсутствии NaN > 30%.
  - Вынести `_one_team` из `_add_lineup_features` в отдельный статический метод класса для улучшения читаемости и тестируемости.
  - Заменить линейный поиск в StreakFeatureGenerator (`long["id"] == mid`) на предварительный словарь `{id: [away_idx, ...]}` — O(1) lookup.
