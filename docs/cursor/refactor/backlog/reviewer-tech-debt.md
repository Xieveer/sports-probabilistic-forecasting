...
- **Ограничения и компромиссы:**
  - **Полный операционный прогон на NHL interim parquet** по-прежнему не зафиксирован в CI (в репозитории нет `data/`); локально: `features_build` + `train` на `nhl_train` при наличии данных.
...
### 2026-05-09 — R30: расхождение «y по голам» vs `y_true` в trace (расследование)

- **Задача:** `backlog/R30.md` (инвариант таргета + trace)
- **Ограничения и компромиссы:**
  - На полном `train_long.parquet` (41120 строк, hydra `nhl_train` + `winner_withOT`) проверка `compute_target_from_market_spec` vs `(pl_goals_full > opp_goals_full)` дала **0 расхождений** — баг в «сыром таргете из кода» на этих данных не подтверждён.
  - Визуальное расхождение на скриншоте может быть из-за **Excel** (счёт `4-1` как дата, смещение строк, неверная формула `y`) или устаревшего CSV.
- **Возможные улучшения / техдолг:**
  - Реализовать R30.1–R30.4: явная колонка `target_from_goals`, assert при записи trace, тесты на parquet/holdout, документация по Excel.
