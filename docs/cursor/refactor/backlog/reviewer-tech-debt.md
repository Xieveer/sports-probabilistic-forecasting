...
- **Задача:** эпик **R37**, подзадача **R37.1** → см. `done_task/R37.1.md` (карточка `backlog/R37.md` удалена после закрытия эпика).
- **Ограничения и компромиссы:**
...
- **Задача:** эпик **R37**, подзадача **R37.3** → `done_task/R37.3.md` (эпик закрыт в `todo-refactor.md`).
- **Ограничения и компромиссы:**
...
- **Задача:** эпик **R37**, подзадача **R37.4** → `done_task/R37.4.md` (эпик закрыт в `todo-refactor.md`).
- **Ограничения и компромиссы:**
...
- **Задача:** эпик **R37**, подзадача **R37.5** → `done_task/R37.5.md` (эпик закрыт в `todo-refactor.md`).
- **Ограничения и компромиссы:**
...
- **Задача:** эпик **R37**, подзадача **R37.6** → `done_task/R37.6.md` (эпик закрыт в `todo-refactor.md`).
- **Ограничения и компромиссы:**
...
### 2026-05-14 — R39.4: CLI post-refresh digest + Telegram HTTP

- **Задача:** `backlog/R39.md` (подзадача **R39.4**) → `done_task/R39.4.md` (эпик R39 остаётся 🟡 в `todo-refactor.md`).
- **Ограничения и компромиссы:**
  - Для отправки используется **первый** `chat_id` из `BOT_ALLOWED_USER_IDS` (как в runbook); мультичат и явный `--chat-id` не вводились.
  - `telegram_send_message` обрезает текст до 4090 символов без разбиения на несколько сообщений; `build_post_refresh_digest_text` уже укладывает лимит, остаётся двойной «зажим».
...
- **Ограничения и компромиссы:**
  ...
  - Повторные Telegram при retry Airflow возможны **без** опции `SF_TELEGRAM_DIGEST_DEDUP` и маркера (см. R39.7).
- **Возможные улучшения / техдолг:**
  ...
  - Операционно: включить dedup в окружении digest по runbook (R39.7); для мульти-воркера — см. ограничения в `done_task/R39.7.md` / запись R39.7 ниже.

### 2026-05-14 — R39.7: observability digest + опциональный dedup маркер

- **Задача:** `backlog/R39.md` (подзадача **R39.7**) → `done_task/R39.7.md` (эпик R39 остаётся 🟡 в `todo-refactor.md`).
- **Ограничения и компромиссы:**
  - Dedup реализован **локальным файлом** в `<project_root>/.cache/digest_telegram_sent/`; при нескольких воркерах без общего FS или при ручной очистке кэша семантика «один send на run» не гарантируется автоматически.
  - Маркеры не имеют TTL: каталог может расти; при сбое записи маркера после успешного send возможен повторный пост (логируется как warning).
  - Сводка `live_odds_status` в логе ограничена топ-N различными значениями (остаток агрегируется) — полный перечень не журналируется.
- **Возможные улучшения / техдолг:**
...
### 2026-05-13 — R40 Codebase housekeeping

- **Задача:** `backlog/R40.md` → `done_task/R40.md`
- **Ограничения и компромиссы:**
  - `check_model_quality` по-прежнему **всегда возвращает True**; изменены только формулировки docstring/логов. Реальная проверка MLflow/Evidently вне объёма R40.
  - Модули `monitoring/drift|performance|ab_testing` **не удалены** (тесты + Sphinx); в Airflow по-прежнему используются gates из `validation.gates`, а не эти классы.
  - Свертка тройных `# ====` выполнена не во всех тестовых файлах с короткими разделителями — сознательно ограниченный дифф.
- **Возможные улучшения / техдолг:**
