...
- **Ограничения и компромиссы:**
  - Scheme B не запускалась в полном объёме по всем 8 турнирам (ресурсоёмко); верификация проведена только на `nhl_train` smoke с `n_resamples=100`. Команды для остальных турниров — незапущенные шаблоны; корректность их конфигов (наличие odds, нужных `market`/`market_spec`) не проверялась автоматически.
...
- **Возможные улучшения / техдолг:**
  ...
  - Smoke CI (GitHub Actions): выполнять `--cfg job --resolve` для Scheme A + Scheme B на `nhl_train` при каждом PR — гарантия, что overrides не сломались после изменений в `conf/`.
