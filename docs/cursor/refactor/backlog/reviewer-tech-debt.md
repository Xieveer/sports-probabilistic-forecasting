...
- **Ограничения и компромиссы:**
  ...
  - CSV `test_bet_trace.csv`, сгенерированный до фикса, остаётся исторически неверным относительно голов — перезапустить `train` для нового артефакта.
- **Возможные улучшения / техдолг:**
  - R30.1–R30.4: явная колонка `target_from_goals`, assert при записи trace, интеграционный тест по CSV, документация Excel.
