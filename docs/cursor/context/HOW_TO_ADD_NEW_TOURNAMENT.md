...
### Каталог портфеля: реализованная граница и следующий этап

После создания `source` и `tournament` добавьте запись в
`conf/portfolio/default.yaml`. Каталог связывает конкретный `tournament` с
`model_pool`, доступными `market_specs` и `deployment_profile`; его загрузчик
fail-fast проверяет совместимость спорта и единственность пула для каждого
`market/spec`. Профиль `candidate` означает, что модель ещё не допускается к
регулярной выдаче. Для `production` обязательны неизменяемая ссылка на модель и
отчёт кандидата.

На момент TASK-003-1 каталог **не запускает** DVC или Airflow автоматически:
действующая процедура ниже со статическим списком в `dvc.yaml` остаётся
актуальной. TASK-003-4 заменит эту ручную регистрацию на orchestration из
каталога. Подробные границы и миграция описаны в
`docs/architecture/adr/ADR-003-configured-multisport-portfolio.md`.
