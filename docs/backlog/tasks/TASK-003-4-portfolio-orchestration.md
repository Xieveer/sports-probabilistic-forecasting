...
- [x] Добавление активного поддерживаемого турнира в каталог обнаруживается
  orchestration contract без изменения Python/DAG списка.
- [x] Команда/DAG строит изолированную heavy-цепочку для одного турнира;
  повтор и конфликтующий refresh одного ключа не создают гонку.
- [x] Dev/CI DVC путь остаётся воспроизводимым и не подменяется production DAG.
