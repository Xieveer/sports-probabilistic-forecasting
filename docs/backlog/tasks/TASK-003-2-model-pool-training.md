...
- [x] Датасеты двух турниров одного пула объединяются только после проверки
  совместимости контракта и сохраняют tournament provenance каждой строки.
- [x] Запуск winner создаёт один model identity пула и отчёт со всеми
  обязательными метриками REQ-003.
- [x] Несовместимый формат/спорт не объединяется и не создаёт артефакт.
