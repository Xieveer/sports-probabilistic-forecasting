...
- [x] Версия хранит состояние состава, источник, время получения, model pool и
  model version.
- [x] Повтор того же confirmed-lineup event не создаёт новую версию или
  дублирующее уведомление.
- [x] Отсутствие/неполнота confirmed состава не инициирует fast path; после
  успешной DB записи сбой Telegram приводит к повтору доставки без пересчёта.
- [x] Интеграционная проверка с локальным адаптером подтверждает путь в пределах
  одной минуты.
