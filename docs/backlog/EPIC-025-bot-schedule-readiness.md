...
Следующие зависимые срезы: персистентное schedule и dispatcher;
Telegram admin/уведомления/e2e;
Operations readiness и выпуск. TASK создаются по мере завершения предыдущего
среза, чтобы не фиксировать неподтверждённые implementation details заранее.
