...
#
# NOTE: Эти колонки захардкожены намеренно, так как определяются архитектурой
# пайплайна, а не бизнес-логикой конкретного турнира.
#
# Рациональ:
# 1. Стабильность: системные колонки не должны меняться через конфиг
# 2. Производительность: нет лишних загрузок конфига при каждом вызове
# 3. Явность: сразу видно какие колонки системные
# 4. Простота: единое место определения для всей системы
#
# Если в будущем потребуется гибкость по турнирам (например, разные источники
# данных с разными мета-колонками), можно вынести в conf/column_categories.yaml
META_COLUMNS = {
    # Общие идентификаторы (системные, стабильные)
    "id",
    "datetime",
    "tournament",
    "status",
    # Long format специфичные (архитектура long_format.py)
    "pl",  # player (текущий игрок в строке)
    "opp",  # opponent
    "side",  # 'h' или 'a'
    "is_home",  # 1 или 0
    # Wide format специфичные (стандартизированные имена участников)
    "home_team",
    "away_team",
    # Турнирные мета-данные (используются для контекста, не для обучения)
    "tour_name",
    "tour_name_en",
    "_tour_name",
