"""Тесты Parquet-store линий (:mod:`sports_forecast.data.providers.odds.store`), схема V2 + миграция V1."""
...
def test_load_v2_parquet_no_migration(tmp_path) -> None:
    path = tmp_path / "v2.parquet"
    save_odds_store(pd.DataFrame([_sample_row_v2()]), path)
    loaded = load_odds_store(path)
    assert loaded["commence_time_utc"].iloc[0] == "2024-01-10T20:00:00+00:00"


def test_load_save_roundtrip_v2(tmp_path) -> None:
...
def test_upsert_dedup_newer_fetched_wins_v2() -> None:
    key = _sample_row_v2(
        close_home=1.0,
...
def test_upsert_new_v1_merged_with_existing_v2() -> None:
    existing = pd.DataFrame([_sample_row_v2(close_home=1.1)])
...
def test_upsert_dedup_equal_fetched_last_row_wins() -> None:
    ...
    old = _sample_row_v2(close_home=1.0, fetched_at=ts)
    new_row = {
        **_sample_row_v2(close_home=2.0, fetched_at=ts),
...
def test_max_game_date_in_store() -> None:
    df = pd.DataFrame(
        [
            _sample_row_v2(game_date="2024-01-05"),
            _sample_row_v2(game_date="2024-02-10"),
        ]
    )
...
def test_upsert_empty_new_returns_existing() -> None:
    existing = pd.DataFrame([_sample_row_v2()])
...
def test_upsert_file_from_missing_store_v2(tmp_path) -> None:
    p = tmp_path / "d" / "pinnacle_odds.parquet"
    new = pd.DataFrame([_sample_row_v2(fetched_at="2024-01-10T00:00:00+00:00")])
...
def test_upsert_file_merges_with_disk(tmp_path) -> None:
    ...
    first = pd.DataFrame([_sample_row_v2(fetched_at="2024-01-01T00:00:00+00:00")])
    save_odds_store(first, p)
    second = pd.DataFrame(
        [
            {
                **_sample_row_v2(fetched_at="2024-01-20T00:00:00+00:00"),
...
def test_upsert_new_partial_nan_fetched_at() -> None:
    row = _sample_row_v2()
    row2 = {**_sample_row_v2(close_home=9.0), "fetched_at": pd.NA}
