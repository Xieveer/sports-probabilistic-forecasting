...
Команда `/upcoming` открывает календарь NHL: выберите «Сегодня», «Завтра», 3, 7, 14
или 30 букмекерских суток. Сутки идут с 08:00 до 08:00 МСК; до 08:00 «Сегодня»
показывает остаток уже начавшегося окна. Бот показывает известные будущие события
независимо от наличия прогноза и коэффициентов, их отдельные состояния и общий статус.
Для пустого периода бот различает подтверждённую пустоту, неполное или устаревшее
покрытие и ошибку обновления. В одном ответе показывается до 50 событий с общим
количеством, остаток списка явно обозначается.
...
или `failed`. Для odds отдельно доступны время последнего подтверждённого
наблюдения и время последней попытки сбора; неуспешный запрос помечает только
события в его временном окне после срока подготовки. Успешный сбор без линии
оставляет состояние `missing`.
